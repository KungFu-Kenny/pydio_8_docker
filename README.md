...
after it finishes up go to this address `http://localhost:80` or `https://localhost:443`( you can change the port in the docker-compose.yml it will look like this **2345**:80 you can change the **2345** part )
...
There's a self signed certificate but if you want to use any other certificate feel free to do so, you can modify the settings as explained by the following you can find them in this directory `/etc/apache2/ssl`you will find both the .crt & .key files.
